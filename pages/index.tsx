...
import { Heart, ChevronDown, Menu, BookOpen, Volume2, VolumeX, Bell } from 'lucide-react';

// --- Theme & Configuration ---
const COLORS = {
  primary: '#A9A3D9', // Dusty Lavender
  background: '#141316', // Deep Warm Gray
  surface: 'rgba(28, 27, 31, 0.7)',
  text: '#F8F5F0', // Cream White
  accent: '#E8DDCF', // Warm Beige
};
...
const GlassCard = ({ children, onClick, className = "", isLocked = false }: { children: React.ReactNode, onClick?: () => void, className?: string, isLocked?: boolean }) => (
  <motion.div
    whileTap={!isLocked ? { scale: 0.98 } : {}}
    onClick={onClick}
    className={`relative overflow-hidden rounded-3xl border border-white/10 bg-white/5 backdrop-blur-xl transition-all duration-500 ${className} ${isLocked ? 'opacity-60 grayscale cursor-not-allowed' : 'cursor-pointer hover:bg-white/10'}`}
  >
    {children}
  </motion.div>
);
...
const DayCard = ({ data, index }: { data: any, index: number }) => {
  ...
  return (
    <div className="w-full mb-6 px-4">
      <GlassCard 
        onClick={() => !data.locked && setIsOpen(!isOpen)}
        isLocked={data.locked}
        className="p-6"
      >
        <div className="flex justify-between items-center mb-2">
          <span className="text-xs uppercase tracking-widest text-primary/80 font-medium">Day {data.day < 10 ? `0${data.day}` : data.day}</span>
        ...
        {data.locked ? (
          ...
          <div className="mt-4">
            <p className="text-sm text-text/70 italic mb-6">&ldquo;Waiting for the most beautiful soul to celebrate another year of life. The best is yet to come.&rdquo;</p>
  ...
  return (
    <div className="min-h-screen bg-[#141316] text-[#F8F5F0] selection:bg-primary/30 font-sans overflow-x-hidden">
      ...
      <section className="relative h-screen flex flex-col justify-center items-center text-center px-8">
        ...
        >
          <h2 className="text-5xl md:text-6xl font-serif mb-6 leading-tight italic">10 Days to <br/> 20th May</h2>
          <p className="text-lg text-text/80 leading-relaxed font-serif italic px-4">
            &ldquo;To my babe, you probably have no idea just how amazing you are. But even if you know, I&apos;m here to tell you again xo.&rdquo;
          </p>
          <motion.div 
            animate={{ y: [0, 10, 0] }}
        ...
        <section className="mt-20 px-6 text-center space-y-12">
          <div className="space-y-6">
             <p className="text-lg leading-relaxed font-serif text-text/90 italic">
              &ldquo;Thank you for being so many things to me; consultant, entertainer, therapist, supporter, sponsor, motivator, audience, teacher... I can go on and on my babe but you&apos;ve tried for me, God bless you always.&rdquo;
            </p>
      ...
      <div className="fixed bottom-0 left-0 w-full z-50 px-6 pb-8 pointer-events-none">
        <div className="max-w-md mx-auto pointer-events-auto">
          <GlassCard className="flex justify-around items-center py-4 px-2 rounded-full border-white/5 bg-surface/80">
            <button className="p-3 bg-primary/20 text-primary rounded-full"><BookOpen className="w-5 h-5" /></button>
            <button 
              onClick={() => setIsMuted(!isMuted)}
              className="p-3 rounded-full hover:bg-white/5 text-text/60"
            >
              {isMuted ? <VolumeX className="w-5 h-5" /> : <Volume2 className="w-5 h-5" />}
            </button>
            <button className="p-3 rounded-full hover:bg-white/5 text-text/60"><Heart className="w-5 h-5" /></button>
          </GlassCard>
        </div>
      </div>
    </div>
